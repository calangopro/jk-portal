...
✅ **Link da bio (24/09):** `jkaliancas.com.br/bio`, em `src/app/(bio)/`, sem cabeçalho nem rodapé do site. A bio é um objeto JSON (`lib/bio/tipos.ts`, versão 2) lido de `site_settings` na chave `pagina:bio`, com a bio de fábrica como padrão. São VERSÕES INTEIRAS: a Normal e uma por campanha (Esquenta, Black, e qualquer outra criada no painel), cada uma com nome, visual, datas e os próprios blocos. No dia, vale a campanha ligada que cobre a data; duas no mesmo dia, vence a mais curta. A primeira versão tinha blocos soltos com data numa lista só, e a JK achou desorganizado: não voltar a esse desenho. O fundo é chapado, sem degradê no topo, a pedido da JK. **Campanhas de data prontas (24/09):** Aniversário JK (01 a 08/11, vinho com confete, vence a Black nessa semana por ser mais curta e por isso carrega as peças da Black), Natal (01 a 25/12, areia, vinho e verde, bolas penduradas, flocos e chapéu de Papai Noel no canto dos produtos) e Ano novo (26/12 a 06/01, branco, areia e dourado, bolinhas subindo). Enfeites em CSS puro, com sorteio de semente fixa e ATRÁS do conteúdo (`components/bio/Enfeites.tsx`): por cima, cortavam o texto da oferta. **Origem sem link etiquetado:** o link divulgado é só `jkaliancas.com.br/bio`; `OrigemDaBio` reconhece Instagram, Threads, TikTok e Facebook pelo user agent, Pinterest e YouTube pela referência, aceita `?o=canal` para o resto e entrega ao GA4 como UTM pelo `page_location` (a barra fica limpa). Anúncio mantém a etiqueta do gerenciador e ganha `utm_term=bio`. Por isso os links internos da bio são `<a>` comuns: numa troca de página sem recarregar o GA4 repetiria o endereço da bio. **Gravar a bio direto no banco não aparece no ar** enquanto a entrada do `unstable_cache` antiga existir (`revalidate: false`): troque a versão em `keyParts` de `lib/bio/ler.ts` no mesmo deploy, ou publique pelo painel. **Preço ao vivo da loja**, pela busca pública da Tray (`buscarNaLoja`, 5 minutos de cache), e nunca da tabela `products`, que só muda quando alguém sincroniza. `category_id` na busca traz a categoria como secundária também, que é o caso das categorias de campanha. Eventos com o `item_id` da Tray, o mesmo que a loja manda ao GA4 na compra. Prévia por data em `/guias/bio/previa?data=AAAA-MM-DD`, só fora da produção. Painel em `/admin/bio`, com abas por versão e prévia ao vivo. **Falta:** tabela de leads e o `/api/bio/lead` (o formulário já chama), e o repasse ao RD Station e ao webhook.
