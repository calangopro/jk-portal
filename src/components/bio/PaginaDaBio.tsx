...
/**
 * A bio de um dia, com os dados buscados no servidor.
 *
...
export async function PaginaDaBio({ bio, hoje }: { bio: Bio; hoje: string }) {
  const versao = versaoDoDia(bio, hoje);

  // Só busca produto da versão que está no ar: a vitrine da Black não gasta
  // pedido à loja em outubro.
  const vitrines = versao.blocos.filter((b) => b.tipo === "vitrine");
  const [produtos, lojas] = await Promise.all([
    Promise.all(vitrines.map((b) => produtosDaVitrine(b.fonte, b.limite))),
    versao.blocos.some((b) => b.tipo === "lojas") ? getPublishedLocations() : Promise.resolve([]),
  ]);
  ...
  return (
    <>
      ...
      <style>{`html,body{background:${corDaBarra(versao.tema)}}`}</style>
      <CorpoDaBio cabecalho={bio.cabecalho} versao={versao} dados={dados} />
    </>
  );
}
